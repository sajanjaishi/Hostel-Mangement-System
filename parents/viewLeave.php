...
session_start();
	...
	<?php
		$id=@$_REQUEST["pid"];
			...
			if( $id!=NULL && $pre!=Null )
			{	
				$query="update leave_module set P_Status='R',rejectReason='$pre' where LeaveID='$id'";
			...
			if( $id!=NULL && $pre==NULL )
			{
				$query="update leave_module set P_Status='A' where LeaveID='$id'";
				if ($con->query($query) === TRUE) 
				{
    ...
    <body class="main_body">
        ...
        <div class="container apply_leave_block">
            <div>
                <h2 class="heading_underline_effect_a heading_underline_effect1 text-center">Leave module</h2><hr/>
                <br/>   
                <div class="tab-content">
                    ...
                    <div class="">
						...
						<?php
								...
								$query="select * from leave_module,parents_registration_master where parents_registration_master.Student_username=leave_module.Username and parents_registration_master.Username='$uname'";
							...
							if($result= $con->query($query))
							{
								...
								{
									$i++;
									echo "<tr><td>".$i."</td><td>".$row["Student_username"]."</td><td>".$row["FromDateTime"]."</td>
									<td>".$row["ToDateTime"]."</td><td>".$row["AppliedDate"]."</td><td>".$row["Reason"]."</td><td>".$row["P_Status"]."</td><td>".$row["W_Status"]."</td><td>".$row["rejectReason"]."</td>";
									if($row["P_Status"]=="P" && $row["W_Status"]=="P")
									{
									 echo "<td><a href='viewLeave.php?pid=".$row["LeaveID"]."' id='a' value='".$row["LeaveID"]."'>Accpect</a></td><td><a href='viewLeave.php?pid=".$row["LeaveID"]."&pre=Parent Reject' id='b' value='".$row["LeaveID"]."'>Reject</a></td>";
									}
									else{ echo "<td>-</td>";}
									echo "</tr>";
								}
